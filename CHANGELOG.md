...
- **Settings scroll-snap now uses M3E `motionScheme.defaultSpatialSpec()`** — the snap animation that flicks the collapsing settings header fully open or closed was using a hard-coded `spring(stiffness = StiffnessMediumLow)` instead of the expressive motion scheme's spatial spec, making it feel inconsistent with the home screen.
